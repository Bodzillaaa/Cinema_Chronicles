...
const WatchList = () => {
  ...
  useEffect(() => {
    const fetchWatchlist = async () => {
      try {
        ...
        const movieDetailsPromises = response.data.content.map((movie) =>
          axios.get(`/api/movie/details/${movie.movie_id}`)
        );
  ...
  return (
    <>
      <Navbar />
      <Container>
        <Heading mt={8}>{user?.user?.first_name}&apos;s watchlist</Heading>
        <Grid
          templateColumns={{
            base: "repeat(1, 1fr)",
            md: "repeat(2, 1fr)",
